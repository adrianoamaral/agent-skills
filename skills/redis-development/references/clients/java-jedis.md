...
This reference covers the `FT.*` (Redis Search) surface of the Jedis client. It shows how Jedis *expresses* the canonical CLI form — it does not re-explain the query DSL. Read it after a rule that already states *what* to do.
...
- **redis-py (Python) equivalents** for the same operations: [`python-redis-py.md`](./python-redis-py.md). Read at most one client reference per task — see the router in [`../README.md`](../README.md).
...
CLI form (from rule `search-index-creation.md`):
...
For pipeline-stage ordering rules, see rule `search-aggregate-pipeline.md`. This section shows only the Jedis builder shape.
...
For lifecycle rules and when to use cursors, see rule `search-aggregate-cursors.md`.
...
For query-attribute syntax (`=>[KNN ...]`, `[VECTOR_RANGE ...]`) and pre-filter shape, read rule `search-vector-query.md`.
...
**Version gate:** `FT.HYBRID` requires Redis ≥ **8.4.0**. On older Redis use the pre-filter + KNN pattern in §10. See rule `search-command-selection.md` for the SEARCH vs AGGREGATE vs HYBRID decision.
...
For interpreting `FT.EXPLAIN` and `FT.PROFILE` output, see rule `search-debugging.md`.
...
For semantics (FT.ALTER capacity, alias use cases), see rule `search-index-management.md`.
