...
This reference covers the `FT.*` (Redis Search) surface of the raw `redis-py` client. It shows how `redis-py` *expresses* the canonical CLI form — it does not re-explain the query DSL. Read it after a rule that already states *what* to do.
...
- **Jedis (Java) equivalents** for the same operations: [`java-jedis.md`](./java-jedis.md). Read at most one client reference per task — see the router in [`../README.md`](../README.md).
...
For pipeline-stage ordering rules, see rule `search-aggregate-pipeline.md`. This section shows only the `redis-py` builder shape.
...
**Version gate:** `FT.HYBRID` requires Redis ≥ **8.4.0**. On older Redis use the pre-filter + KNN pattern in §9. See rule `search-command-selection.md` for the SEARCH vs AGGREGATE vs HYBRID decision.
